...
public class cameralook : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (shouldRotate)
        {
            samera.transform.rotation = Quaternion.Lerp(transform.rotation, desiredRotation, rotationSpeed * Time.deltaTime);

            // Stop rotating when close enough
            if (Quaternion.Angle(transform.rotation, desiredRotation) < 0.1f)
            {
                samera.transform.rotation = desiredRotation;
                shouldRotate = false;
            }
        }
    }
